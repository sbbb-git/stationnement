...
#
# Le comportement de la veille est éprouvé dans tests/test_veille.py, où ses
# scripts sont réellement exécutés. Ne reste ici que ce qui tient au calendrier.
...
def test_la_veille_se_reveille_plusieurs_fois_la_nuit():
    """Un relais manqué se juge après minuit : il faut des réveils entre
    minuit et 9 h, et plusieurs, puisque GitHub en saute les trois quarts."""
    ...
    for mois in (7, 1):
        ...
        for entree in declencheurs["schedule"]:
            ...
            for h in _expand(heure_champ, 23):
                for m in _expand(minutes, 59):
                    heures.add(datetime(2025, mois, 15, h, m, tzinfo=UTC)
                               .astimezone(PARIS).strftime("%H:%M"))
        nuit = [h for h in heures if "00:00" <= h <= "08:59"]
        assert len(nuit) >= 3, f"mois {mois} : {sorted(nuit)}"
    assert "workflow_dispatch" in declencheurs


def test_aucune_action_ne_tourne_encore_sur_node_20():
    """GitHub a retiré Node 20 de ses machines le 23/09/2026. Les actions qui le
    déclaraient tournaient depuis en mode forcé sur Node 24 — un sursis, pas
    une garantie. Premières versions Node 24 : checkout v5, setup-python v6,
    cache v5, github-script v8."""
    minimum = {"actions/checkout": 5, "actions/setup-python": 6,
               "actions/cache": 5, "actions/github-script": 8}
    for fichier in sorted(ROOT.glob(".github/workflows/*.yml")):
        for action, version in re.findall(r"uses:\s*([\w-]+/[\w-]+)@v(\d+)",
                                          fichier.read_text(encoding="utf-8")):
            if action in minimum:
                assert int(version) >= minimum[action], \
                    f"{fichier.name} : {action}@v{version} tourne encore sur Node 20"
